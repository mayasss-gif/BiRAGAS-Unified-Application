...
2) Root-dir mode:
   python pathway_classifier.py --root-dir <MDP_OUT_ROOT or disease_dir>

   - If <root-dir> looks like an mdp_pipeline_3 OUT_ROOT:
       For each disease folder (excluding baseline_consensus, comparison, results, agentic_analysis):
           * Require:
               - gsea_prerank.tsv
             Then (for that disease):
               1) classify gsea_prerank.tsv
               2) classify core_enrich_up.csv / core_enrich_down.csv if present

   - If <root-dir> itself looks like a single disease folder (contains gsea_prerank.tsv):
       * Process that folder directly with the same logic.

   - Apply mild significance filters:
       - core_enrich_*: qval <= q_thresh_core (default 0.2), cap max_rows
       - gsea_prerank: FDR/q <= q_thresh_gsea (default 0.3), cap max_rows
         (robust to column names like FDR q-val, FDR_q, FDR.q.val, padj, etc.)
...
def _guess_column(cols: Dict[str, str], candidates: List[str]) -> Optional[str]:
    """
    Given a dict {lower_name: original_name}, return the original name
    of the first candidate that exists (case-insensitive).
    """
...
def _filter_enrichment_for_classification(
    ...
    Robust to different column namings, e.g.:
      - term / Term / NAME / pathway
      - FDR q-val / FDR_q / FDR.q.val / padj / qval / pval
    """
    df = df.copy()
    ...
    if kind == "gsea":
        ...
        if not termcol or not qcol:
            logger.warning(
                "gsea_prerank missing a recognizable term/FDR column – "
                f"got columns={list(df.columns)}; tried term~{['term','name','pathway','gene set','geneset']} "
                f"and q~{['FDR q-val','FDR_q','FDR.q.val','FDR q','FDR','padj','adj p','adj_p','adj p-val','qval','q-value']}. "
                "Skipping filter and just taking head(max_rows)."
            )
            return df.head(max_rows)

        logger.info(f"Using '{termcol}' as term column and '{qcol}' as FDR/q column for gsea_prerank")

        # Coerce q column to numeric
        ...
        if df_f.empty:
            ...
            if len(df_f) > max_rows:
                ...
                nescol = _guess_column(cols, ["nes", "nes score"])
                if nescol and nescol in df_f.columns:
    ...
    if not termcol:
        logger.warning(
            "core_enrich file missing a recognizable term column – "
            f"got columns={list(df.columns)}; tried {['term','name','pathway']}. "
            "Skipping filter and just taking head(max_rows)."
        )
        return df.head(max_rows)
    ...
    if not qcol:
        logger.warning(
            "core_enrich file missing a recognizable q/p column – "
            f"got columns={list(df.columns)}; tried "
            f"{['qval','q-value','q value','fdr','fdr_q','fdr q-val','padj','pval','p-value','p value']}. "
            "Skipping filter and just taking head(max_rows)."
        )
        return df.head(max_rows)

    logger.info(f"Using '{termcol}' as term column and '{qcol}' as q/p column for core_enrich")
...
def _process_one_disease_dir(
    ...
    """
    Process ONE disease directory:

      - Require gsea_prerank.tsv to exist; if not, skip the disease completely.
      - If gsea_prerank.tsv is present:
          1) classify gsea_prerank.tsv
          2) then classify core_enrich_up.csv / core_enrich_down.csv if they exist.
    """
    written: List[Path] = []
    ...
    # --- HARD GATE: require gsea_prerank.tsv in this disease dir ---
    if not gsea_path.exists():
        logger.info(
            f"  No gsea_prerank.tsv in {ddir} – skipping this disease "
            "(and not touching core_enrich_up/down)."
        )
        return written

    # 1) Classify gsea_prerank.tsv first
    ...
    if out_gsea is not None:
        ...
        logger.warning(
            f"  gsea_prerank.tsv present but could not be classified in {ddir.name}. "
            "core_enrich_up/down will still be attempted."
        )

    # 2) Then classify core_enrich_up/down *if they exist*
    if core_up_path.exists():
        ...
        logger.info(f"  No core_enrich_up.csv in {ddir.name}")

    if core_down_path.exists():
        ...
        logger.info(f"  No core_enrich_down.csv in {ddir.name}")
...
def categorize_disease_enrichment_files(
    ...
    """
    ROOT-DIR MODE (supports OUT_ROOT and single-disease dirs):
    ...
    1) OUT_ROOT-style:
       ...
       -> root_dir has NO gsea_prerank.tsv, but its subfolders do.
          We process each disease subfolder.

    2) Single-disease-style:
         root_dir = .../GL_enrich/Breast_Cancer
             gsea_prerank.tsv
             core_enrich_*.csv (optional)
             ...

       -> root_dir ITSELF is treated as one disease folder.

    In ALL cases:
      - A directory is processed ONLY IF its own gsea_prerank.tsv exists.
      - core_enrich_up/down are classified only AFTER gsea_prerank.tsv.
    """
    ...
    # --- Case 2: root_dir itself looks like a disease folder (GL_enrich/Breast_Cancer) ---
    root_has_gsea = (root_dir / "gsea_prerank.tsv").exists()
    if root_has_gsea:
        logger.info(
            f"Root dir {root_dir} looks like a single disease folder "
            "(found gsea_prerank.tsv) – processing it directly."
        )
        written.extend(
            _process_one_disease_dir(
                root_dir,
                q_thresh_core=q_thresh_core,
                q_thresh_gsea=q_thresh_gsea,
                max_rows=max_rows,
        ...
        return written  # single-disease mode done

    # --- Case 1: OUT_ROOT-style – look for disease subfolders ---
    disease_dirs = list_disease_folders(root_dir)
    if not disease_dirs:
        logger.warning(
            f"No disease folders with gsea_prerank.tsv found under {root_dir} "
            "(root also has no gsea_prerank.tsv). Nothing to classify."
        )
        return written

    logger.info(f"Found {len(disease_dirs)} candidate disease folder(s) under {root_dir}")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description=(
            ...
            "    under an mdp_pipeline_3 OUT_ROOT or a single disease dir (--root-dir)."
        ),
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "or a single disease folder (with gsea_prerank.tsv and/or core_enrich_up/down.csv)."
        ),
    )
    parser.add_argument(
        "--input",
